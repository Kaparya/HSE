;RANDOM SIZE OF RANDOM COLOR SQUARES 
;To stop the programm use "(fileName)/off"
...
rand8 endp
    ...
    mov BH, AH
    shl BH, 4
    ;mov BH, 40h         ; Атрибут черный по серому (первое число - RGB палитра фона/ второе - текста)
    
    call rand8 ;координата Y с 1 до 11
    mov CL, 11
    div CL
    inc AH
    mov CH, AH   ; Ко-
    
    call rand8 ;координата X с 1 до 39
    mov CL, 39
    div CL
    inc AH
    mov CL, AH; ор- 
    ...
    call rand8 ;координата Y с 12 до 23
    mov CL, 12
    div CL
    add AH, 12
    mov DH, AH    ; ди-
    
    call rand8 ;координата X с 40 до 78
    mov CL, 39
    div CL
    add AH, 40
...
;----------------------------------------------------------------------------
    flag    DB 0
    high_Y  DB 03; координаты окна
    left_X  DB 40; координаты окна
    low_Y   DB 19; координаты окна
